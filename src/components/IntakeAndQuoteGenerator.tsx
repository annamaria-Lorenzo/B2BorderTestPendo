...
import { IntakeItem, User, Quote, PaymentTerms, ShippingMethod } from '../types';
...
export const IntakeAndQuoteGenerator: React.FC<IntakeAndQuoteGeneratorProps> = ({
  intakeList,
  currentUser,
  onUpdateQty,
  onRemoveItem,
  ...
  const [shippingMethod, setShippingMethod] = useState<ShippingMethod>('Standard Freight (3-5 Days)');
  ...
  const totalWeightKg = intakeList.reduce((acc, item) => acc + item.totalWeightKg, 0);
  ...
  const estimatedTax = currentUser.taxExemptNo ? 0 : Math.round((subtotal - discountAmount) * 0.06 * 100) / 100;
  const grandTotal = Math.round((subtotal - discountAmount + shippingCost + estimatedTax) * 100) / 100;

  const handleGenerateQuote = async () => {
    if (intakeList.length === 0) {
      setErrorNotice('Intake list is empty. Please add hardware items before generating a quote.');
      return;
    }
  ...
  const handleSubmitOrderFromQuote = async (quoteId: string) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to convert quote to order');
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500">Tax Exempt Ref: {currentUser.taxExemptNo}</span>
          </div>
          <h2 className="text-xl font-bold text-slate-900 mt-1 flex items-center gap-2">
            <ShoppingCart className="w-5 h-5 text-slate-700" /> B2B Purchase Intake List & Automated Quote Generator
          </h2>
        </div>
      ...
      {intakeList.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center text-slate-500 space-y-4 shadow-sm">
          ...
          <h3 className="text-base font-bold text-slate-800">Your B2B intake list is currently empty</h3>
          <p className="text-xs max-w-md mx-auto text-slate-500">
            Select items from our commercial hardware catalog or use our AI Voice Agent to talk about what you need for your furniture assembly projects.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                  <Package className="w-4 h-4 text-slate-700" /> Itemized Component Intake ({intakeList.length} SKUs)
                </h3>
                <span className="text-xs text-slate-500">Total Weight: <strong className="text-slate-800">{totalWeightKg.toFixed(1)} kg</strong></span>
              </div>
                    ...
                    {intakeList.map(item => (
                      <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-3">
                          ...
                          <div className="text-[10px] text-slate-500">UOM: {item.unitOfMeasure}</div>
                          {item.customNotes && (
                        ...
                        <td className="p-3 font-medium text-slate-700">{item.selectedFinish}</td>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
              <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                <Truck className="w-4 h-4 text-slate-700" /> Commercial Logistics & Payment Configuration
              </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  >
                    <option value="Net 30">Net 30 (Approved Commercial Credit)</option>
                    <option value="Net 60">Net 60 (Enterprise Line of Credit)</option>
                    <option value="Pre-paid Credit Card">Pre-paid Commercial Card</option>
                    <option value="Wire Transfer">ACH / Bank Wire Transfer</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-200"
                  >
                    <option value="Standard Freight (3-5 Days)">Standard Commercial Freight (3-5 Days)</option>
                    <option value="Expedited Freight (1-2 Days)">Expedited Air Freight (1-2 Days)</option>
                    <option value="White Glove LTL Delivery">White Glove LTL Liftgate Delivery</option>
                  </select>
                </div>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
              <h3 className="font-bold text-sm text-slate-900 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <Calculator className="w-4 h-4 text-emerald-700" /> Real-time Automated Pricing
                </span>
              ...
              <div className="space-y-2.5 text-xs border-y border-slate-200 py-4 text-slate-600">
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between text-amber-900 font-semibold bg-amber-50 p-2 rounded-lg border border-amber-200">
                    <span className="flex items-center gap-1">
                      <Zap className="w-3.5 h-3.5 text-amber-600" /> Tier Account Discount ({discountPercent}%):
                    </span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-slate-800">
                    {currentUser.taxExemptNo ? '$0.00 (Exempt)' : `$${estimatedTax.toFixed(2)}`}
                  </span>
                </div>
                ...
                <div className="pt-3 border-t border-slate-200 flex justify-between items-center text-sm font-bold text-slate-900">
                  ...
                  <span className="text-emerald-700 font-mono text-base">${grandTotal.toFixed(2)}</span>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                >
                  ...
                  {isGenerating ? 'Generating Official Quote...' : 'Generate Official B2B Quote PDF'}
                </button>
              ...
              <div className="text-[11px] text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                <div className="flex items-center gap-1 text-slate-800 font-semibold">
                  <ShieldCheck className="w-3.5 h-3.5 text-slate-700" /> Net 30 Credit Verification
                </div>
                <p>
                  Available Credit: <strong className="text-emerald-700">${currentUser.creditAvailable.toLocaleString()}</strong>. Standard lead time: 3-5 business days.
                </p>
              </div>
            </div>
          </div>
