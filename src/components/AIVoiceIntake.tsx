...
export const AIVoiceIntake: React.FC<AIVoiceIntakeProps> = ({
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) {
      alert('Speech recognition is not supported in this browser. You can type your request directly into the prompt box.');
      return;
    }
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1 max-w-2xl">
            <div className="flex items-center gap-2">
              <span className="bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-amber-600" /> AI Voice & Speech Intake Agent
              </span>
              <span className="text-slate-400 text-xs hidden sm:inline">Powered by Gemini 3.6 Flash</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">
              Talk or Type Your Commercial Hardware Request
            </h2>
            <p className="text-xs sm:text-sm text-slate-600">
              Speak naturally about your furniture production requirements. Our AI parses technical specs, searches our catalog, applies <strong className="text-slate-900">{currentUser.accountTier}</strong> volume pricing, and automatically updates your intake list.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {speechEnabled ? <Volume2 className="w-4 h-4 text-emerald-600" /> : <VolumeX className="w-4 h-4" />}
              <span className="hidden sm:inline">{speechEnabled ? 'Speech Voice On' : 'Speech Voice Off'}</span>
            </button>
                    ...
                    {msg.extractedItems && msg.extractedItems.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-200 bg-slate-50 p-3 rounded-xl border border-slate-200">
                        <div className="flex items-center gap-1.5 text-xs font-bold text-slate-800 mb-2">
                          ...
                          AI Auto-Extracted Hardware Items ({msg.extractedItems.length}):
                        </div>
                        <div className="space-y-2">
                          {msg.extractedItems.map((item, idx) => (
                            ...
                            >
                              <div>
                                <span className="font-bold text-slate-900">{item.sku}</span> - {item.name}
                                <div className="text-[11px] text-slate-500">
                                  Qty: <strong className="text-slate-800">{item.quantity}</strong> | Finish: <strong className="text-slate-800">{item.finish || 'Standard'}</strong>
                                </div>
                              </div>
                              <span className="text-emerald-700 font-bold">
                                ${item.unitPrice ? (item.unitPrice * item.quantity).toFixed(2) : 'Tier Pricing'}
                              </span>
                            </div>
                          ))}
                        </div>
            ...
            {isLoading && (
              <div className="flex items-center gap-3 text-slate-500 text-xs p-2">
                ...
                <div className="flex items-center gap-1.5 bg-white border border-slate-200 px-3 py-2 rounded-2xl shadow-xs">
                  ...
                  <span className="ml-2">Analyzing technical specs & stock availability...</span>
                </div>
              </div>
            )}
            <div ref={chatEndRef} />
          ...
          <div className="p-4 bg-white border-t border-slate-200 space-y-3">
            ...
            {isListening && (
              <div className="bg-rose-50 border border-rose-200 p-2.5 rounded-xl flex items-center justify-between text-xs text-rose-800 animate-pulse">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold">Microphone Active - Speak clearly now...</span>
                </div>
                <button
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={isListening ? 'Stop Speech Recording' : 'Start Microphone Speech Input'}
              >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
            <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-amber-600" /> Example Voice & Speech Prompts
            </h3>
            <p className="text-xs text-slate-500">
              Click any quick phrase to test AI technical parsing and real-time intake list generation:
            </p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-900 flex items-center gap-2">
                <Layers className="w-4 h-4 text-slate-700" /> Active Account Perks
              </span>
            ...
            <div className="text-xs space-y-2 bg-slate-50 p-3 rounded-xl border border-slate-200">
              <div className="flex justify-between text-slate-600">
                ...
                <strong className="text-amber-800">
                  {currentUser.accountTier.includes('Tier 2') ? '18% Off Catalog' : currentUser.accountTier.includes('Tier 1') ? '10% Off Catalog' : 'Standard'}
                </strong>
              </div>
              <div className="flex justify-between text-slate-600">
                <span>Net 30 Limit:</span>
                <strong className="text-emerald-700">${currentUser.creditLimit.toLocaleString()}</strong>
              </div>
              <div className="flex justify-between text-slate-600">
                <span>Tax Exemption:</span>
                <strong className="text-slate-900">{currentUser.taxExemptNo || 'Active'}</strong>
              </div>
            </div>
