...
export const CustomerAuthModal: React.FC<CustomerAuthModalProps> = ({
  ...
  const handleSubmitSignUp = (e: React.FormEvent) => {
    ...
    // Track B2B account registration
    if ((window as any).pendo) {
      (window as any).pendo.track("b2b_account_registered", {
        company_name: formData.companyName,
        contact_name: formData.name || "",
        account_tier: formData.accountTier,
        has_tax_exemption: !!formData.taxExemptNo,
      });
    }
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Select account profile or register commercial account
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6">
          {activeMode === "switch" ? (
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                Choose a pre-configured B2B account profile to test custom
                volume tier pricing, Net 30 credit limits, and tax-exempt order
                processing:
              </p>
            ...
            <form onSubmit={handleSubmitSignUp} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block text-slate-700 font-medium mb-1 flex items-center gap-1">
                  ...
                  Reseller / State Tax Exemption Certificate #
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, taxExemptNo: e.target.value })
                  }
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-200"
                />
              </div>
              ...
              <div>
                <label className="block text-slate-700 font-medium mb-1">
                  Requested Commercial Volume Tier
                </label>
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      accountTier: e.target.value as UserAccountTier,
                    })
                  }
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-200"
                >
                  <option value="Standard B2B">
                    Standard B2B (MSRP Pricing)
                  </option>
                  <option value="Tier 1 Wholesale (-10%)">
                    Tier 1 Wholesale (10% Off Catalog + Net 30)
                  </option>
                  <option value="Tier 2 Enterprise (-18%)">
                    Tier 2 Enterprise (18% Off Catalog + $75k Credit)
                  </option>
                </select>
              </div>
